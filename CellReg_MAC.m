function CellReg_MAC(footprint_data_archivo,Params)
%% Modificacion de Moises AC

% Esta funcion permite correr el codigo de CellReg sin utilizar el GUI para
% obtener la correlacion de las huellas neuronales.

% Las modificaciones se hacen en el scrtipt FootprintCorrelation
...
maximal_rotation = Params.maximal_rotation;  % in degrees - only relevant if 'Translations and Rotations'
...
Correlation_Threshold = Params.Correlation_Threshold; % Default 0.65. If de initial registration type is Spatial correlation
Distance_Threshold_Microns = Params.Distance_Threshold_Microns; % Default 5. If de initial registration type is Centroid distance
...
Final_p_same_threshold = Params.Final_p_same_threshold; % Default 0.5. Only relevant if probabilistic approach is used
Final_Correlation_Threshold = Params.Final_Correlation_Threshold; % Default 0.65. If de initial registration type is Spatial correlation
Final_Distance_Threshold_Microns = Params.Final_Distance_Threshold_Microns; % Default 5. If de initial registration type is Centroid distance



% Defining the results_directory and creating the figures_directory:
results_directory = path_name; %--MAC 02.dic.2019 
figures_directory=fullfile(results_directory,'Figures');
if exist(figures_directory,'dir')~=7
    mkdir(figures_directory);
end
% figures_visibility='on'; % either 'on' or 'off' (in any case figures are saved) %--MAC 02.dic.2019 
    
file_names = footprint_data_archivo; %--MAC 02.dic.2019 
...
%microns_per_pixel=2.35; %--MAC 02.dic.2019 
...
% alignment_type='Translations and Rotations'; % either 'Translations', 'Translations and Rotations' or 'Non-rigid' %--MAC 02.dic.2019 
use_parallel_processing=true; % either true or false
%maximal_rotation=30; % in degrees - only relevant if 'Translations and Rotations' is used %--MAC 02.dic.2019 
%transformation_smoothness=2; % levels of non-rigid FOV transformation smoothness (range 0.5-3)%--MAC 02.dic.2019 
%reference_session_index=1; %--MAC 02.dic.2019 
...
% maximal_distance=12; % cell-pairs that are more than 12 micrometers apart are assumed to be different cells %--MAC 02.dic.2019 
...
% initial_registration_type=best_model_string; % either 'Spatial correlation', 'Centroid distance', or 'best_model_string'; %--MAC 02.dic.2019 
...
if strcmp(initial_registration_type,'Spatial correlation') % if spatial correlations are used
    if exist('spatial_correlation_intersection','var')
        ...
        initial_threshold = Correlation_Threshold; % a fixed correlation threshold not based on the model %--MAC 03.dic.2019 
    end
    ...
    if exist('centroid_distance_intersection','var')
        ...
        initial_threshold = Distance_Threshold_Microns; % a fixed distance threshold not based on the model %--MAC 03.dic.2019
    end
...
% registration_approach='Probabilistic'; % either 'Probabilistic' or 'Simple threshold' %--MAC 03.dic.2019
% model_type=best_model_string; % either 'Spatial correlation' or 'Centroid distance' %--MAC 03.dic.2019
p_same_threshold = Final_p_same_threshold; % only relevant if probabilistic approach is used %--MAC 03.dic.2019
...
if strcmp(registration_approach,'Simple threshold') % only relevant if a simple threshold is used
    if strcmp(model_type,'Spatial correlation')
        if exist('spatial_correlation_intersection','var')
            ...
            final_threshold = Final_Correlation_Threshold; % a fixed correlation threshold not based on the model %--MAC 03.dic.2019
        end
    elseif strcmp(model_type,'Centroid distance')
        if exist('centroid_distance_intersection','var')
            final_threshold=centroid_distance_intersection; % the threshold for p_same=0.5;
        else
            final_threshold = Final_Distance_Threshold_Microns; % a fixed distance threshold not based on the model %--MAC 03.dic.2019
        end
